...
# 生成请求body体
def getBody(appid, text, width, height, user, fengge):
    text = fengge + text
    body = {
        "header": {
            "app_id": appid,
...
# 发起请求并返回结果
def main(text, appid, apikey, apisecret, width, height, user, fengge):
    ...
    content = getBody(appid, text, width=width, height=height, user=user, fengge=fengge)
    print(time.time())
...
def parser_Message(message, save_path):
    ...
    if code != 0:
        ...
        savePath = os.path.join(save_path, f"{imageName}.jpg")
...
if __name__ == '__main__':
    ...
    APPID = '5277f5fd'
    APISecret = 'ZjIyOGRlMmMyMTcyNDA3YmNkYWUwNjk0'
    APIKEY = 'a866423e3dc77a55ea875cecc6cdb153'
    desc = '''索隆'''
    width = 512
    height = 512
    user = "user"
    res = main(desc, appid=APPID, apikey=APIKEY, apisecret=APISecret, width=width, height=height, user=user)
    # print(res)
    # 保存到指定位置
    # parser_Message(res)
